...
void Listevoitures() {
    FILE* f = fopen("Listevoitures.txt", "r"); //ouvre le fichier "Listevoitures.txt" en mode lecture
    ...
    printf("----------------------------------------\n");
    printf("Reference  |  Nom du produit    |Couleur|   Prix    |  Quantite en stock\n");
    printf("--------------------------------------------------------------------\n");

    while (fscanf(f, "%s %s %s %f %d", reference, nomproduit, couleur, &prix, &quantiteenstock) != EOF) { //parcours le fichier jusqu'à "End Of File" (fin du fichier) en lisant les données de chaque voiture dans l'ordre spécifié
        printf("%-10s | %-18s | %-5s | %.2f  | %d\n", reference, nomproduit, couleur, prix, quantiteenstock); //affiche ces informations 
    }
...
void Listeclients() {
    ...
    f = fopen("clients.txt", "rt"); //ouvre le fichier "clients.txt" en mode lecture de texte
    if (f == NULL) {
        printf("Erreur lors de l'ouverture du fichier.\n"); //message d'erreur si le fichier ne s'ouvre pas
        return;
    }
    
    while (fgets(ligne, CSTE, f) != NULL) { //lit chaque ligne du fichier et la stocke dans la variable "ligne"
...
void Rechercheproduit(Car car[], int numProducts) {
  ...
  printf("-----------------------------\n");
    ...
    while (fscanf(f, "%s %s %s %f %d", c.reference, c.nomproduit, c.couleur, &c.prix, &c.quantiteenstock) != EOF) {
        if (strcmp(c.reference, reference) == 0 && strcmp(c.couleur, couleur) == 0) {
            found = 1;
...
//
int Supprimer(struct Client* cl, int quantite, const char* reference, const char* couleur) {
    FILE* f;
    FILE* temp;
    Car c;
    int found = 0;
    ...
    while (fscanf(f, "%s %s %s %f %d", c.reference, c.nomproduit, c.couleur, &c.prix, &c.quantiteenstock) != EOF) {
        ...
        fprintf(temp, "%s %s %s %.2f %d\n", c.reference, c.nomproduit, c.couleur, c.prix, c.quantiteenstock);
    }
...
void afficherRuptures() {
    ...
    while (fgets(ligne, CSTE, fichier) != NULL) {
        sscanf(ligne, "%s %s %s %f %d", car.reference, car.nomproduit, car.couleur, &car.prix, &car.quantiteenstock);

        if (car.quantiteenstock == 0) {
            printf("%-10s | %-18s | %-7s | %.2f    | %d\n", car.reference, car.nomproduit, car.couleur, car.prix, car.quantiteenstock);
    ...
    for (int i = 0; i < nbProduitsBasStock - 1; i++) {
        for (int j = i + 1; j < nbProduitsBasStock; j++) {
            if (produitsBasStock[i].quantiteenstock > produitsBasStock[j].quantiteenstock) {
    ...
    for (int i = 0; i < nbProduitsBasStock && nbProduitsAffiches < 5; i++) {
        printf("%-10s | %-18s | %-7s | %.2f    | %d\n", produitsBasStock[i].reference, produitsBasStock[i].nomproduit, produitsBasStock[i].couleur, produitsBasStock[i].prix, produitsBasStock[i].quantiteenstock);
        nbProduitsAffiches++;
    }
}
int CapaciteMax = 1000;

int Ajouter(struct Client* cl, int quantite, const char* reference, const char* couleur) {
    FILE* f;
    FILE* temp;
    Car c;
    int found = 0;
...
float CalculerTotal(struct Client* cl) {
    ...
    for (int i = 0; i < cl->nbProduits; i++) {
        FILE* fichier = fopen("Listevoitures.txt", "r"); // Nouveau pointeur de fichier pour chaque itération
        if (fichier == NULL) {
            printf("Erreur lors de l'ouverture du fichier.\n");
            fclose(f);
            return 0;
        ...
        Car c;
        while (fscanf(fichier, "%s %s %s %f %d", c.reference, c.nomproduit, c.couleur, &c.prix, &c.quantiteenstock) != EOF) {
            if (atoi(c.reference) == cl->cart[i]) {
                total += c.prix * cl->quantiteProduits[i];
                total -= 5000 * cl->quantiteProduits[i]; // Soustraire 5000 pour chaque produit acheté
                break;
            }
        }
...
int Acheter(struct Client* cl, int quantite, const char* reference, const char* couleur) {
    FILE* f;
    FILE* temp;
    Car c;
    int found = 0;
    ...
    char achat[CSTE];
    snprintf(achat, sizeof(achat), "Reference: %s, Couleur: %s, Quantite: %d", reference, couleur, quantite);
    if (cl->nbProduits < HISTORIQUE_MAX) {
        ...
        // Décalage des éléments existants vers la droite et écrasement du dernier élément
...
int Supprimerclient(struct Client* cl, const char* nomUtilisateur) {
    ...
    if (reponse == 1) {
        ...
        printf("Le client avec le nom d'utilisateur '%s' a été supprimé avec succès.\n", nomutilisateur);
        return 1;
    }
    ...
    do {
        switch (choixclientco) {
            ...
            case 4:
                client->nbProduits = 0;
                do {
                    printf("Entrez la référence de la voiture que vous souhaitez acheter : ");
                    scanf("%s", reference);
                    getchar();
                ...
                } while (1); // Boucle infinie, sort de la boucle uniquement si la réponse n'est pas "oui"
...
int Newclient() {
    ...
    if (reponse == 1) {
        ...
        fprintf(fichierClients, "%s %s %s\n", client.nom, client.prenom, client.nomutilisateur);
  ...
  printf("\nVous êtes en mode gérant.\n");
    ...
    if (ajoutResult != 1) {
        printf("Échec de l'ajout de nouveaux produits.\n");
          ...
          coGerant();
        case(6):
          printf("Entrez la référence de la voiture que vous souhaitez supprimer : ");
            scanf("%s", reference);
            getchar();
